...
const Contact = () => {
  ...
  const handleInput = () => {
    ...
    if (textarea) {
      ...
      const lineHeight = 24; // Assuming each line is about 24px (adjust if needed)
        ...
        <div className='flex flex-col justify-center items-start gap-2 w-full md:max-w-[330px] max-h-full'>
          {
            items.map((item, index) => (
              <div key={index} className='flex flex-col gap-1 w-full'>
                ...
                <input type={item.type} name={item.name} placeholder={item.placeholder} className='border border-gray-300 px-4 py-2 rounded-lg focus:outline-none text-gray-500 placeholder:text-gray-500/50' />
              </div>
            ))
          }
          <div className='flex flex-col gap-1 w-full'>
            <label htmlFor="message" className='text-gray-600 font-semibold'>Message</label>
            <textarea ref={textareaRef} onInput={handleInput} type="text" name="message" placeholder="Enter your Message" className='border border-gray-300 px-4 py-2 rounded-lg focus:outline-none text-gray-500 placeholder:text-gray-500/50' />
          </div>
          <button className='border border-black px-8 py-4 text-sm hover:bg-black hover:text-white transition-all duration-500 rounded-xl'>Send Message</button>
        </div>
      </div>
